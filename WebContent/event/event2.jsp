    		...
    		<c:forEach var="couponBean" items="${list}" varStatus="status">
				<div class="gallery-grid-item">
	    			<a href="${contextPath}/coupon/eventInfo.do?event_num=${couponBean.event_num}">
	 					<img src="../upload/${couponBean.event_timage}" style="width: 300px; max-width: 100%; padding: 8px;">
	    			</a>
	    			<small>${couponBean.event_title}</small>	
	    			<small>${couponBean.event_startdate} ~ ${couponBean.event_enddate}</small>				
    			</div>	
    		</c:forEach>
    	</c:if>
		<c:if test="${list == null }">
